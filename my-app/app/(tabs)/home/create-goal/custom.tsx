...
export default function BudgetGoalsScreen() {
  ...
  const handleSaveGoal = async (title: string) => {
    ...
    await saveGoal({ id, title, completed: false, icon: 'goals-removebg-preview', iconName: 'leaf'  });
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
        <SafeAreaView style={{ flex: 1 }}>
          ...
          <View style={styles.headerSection}>
            <FontAwesome name="paint-brush" size={90} color="#BAA8E4" style={{ marginTop: 24 }} />
            <Text style={styles.progress}>Create a Custom Goal</Text>
          </View>

          <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
            <View style={styles.card}>
                <Text style={[styles.cardTitles, { marginBottom: 8 }]}>
            Write your own goal in your own words:
            </Text>
            <TextInput style={[styles.input, { height: 100, textAlignVertical: 'top' }]} placeholder="e.g., I want to only use snus after 5 PM."
        multiline value={customGoal} onChangeText={setCustomGoal}/>
    {customGoal.trim() !== '' && (
      <Pressable style={styles.saveButton} onPress={() => handleSaveGoal(customGoal)}>
        <Text style={styles.saveButtonText}>Save this goal</Text>
      </Pressable>
    )}
  </View>
