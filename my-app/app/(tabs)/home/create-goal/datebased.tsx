...
export default function BudgetGoalsScreen() {
  ...
  const handleSaveGoal = async (title: string) => {
    ...
    await saveGoal({ id, title, completed: false, icon: 'goals-removebg-preview', iconName: 'calendar-o'  });
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
        <SafeAreaView style={{ flex: 1 }}>
          ...
          <View style={styles.headerSection}>
            <FontAwesome name="calendar" size={90} color="#5DADE2" style={{ marginTop: 24 }} />
            <Text style={styles.progress}>Create a Date-Based Goal</Text>
          </View>

          <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
            {renderCard('budget1', 'Be snus-free by ___', 'Enter date/weekday/month/week...', 'money', '#5DADE2')}
            {renderCard('budget2', 'Go without snus for ___ days starting ___ ', 'Enter date...', 'money', '#5DADE2')}
          </ScrollView>
        </SafeAreaView>
...
//Go without snus for X days starting X 